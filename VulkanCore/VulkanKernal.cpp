...
namespace VulkanKernal
{
	...
	bool VK::Initialize(HWND hwnd, HINSTANCE hInstance)
	{
		...
		PSO = PipelineState::LoadFromFile(L"", Device, SwapChainExtent, DescriptorSetLayout, RenderPass);
		CreateCommandPool();
	...
	void VK::CleanupSwapChain()
	{
		...
		vkDestroyPipeline(Device, PSO.GraphicsPipeline, nullptr);
		vkDestroyPipelineLayout(Device, PSO.PipelineLayout, nullptr);
	...
	void VK::RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, PSO.GraphicsPipeline);
		...
		vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, PSO.PipelineLayout, 0, 1, &DescriptorSets[CurrentFrame], 0, nullptr);
